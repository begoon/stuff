...
void load_file1(std::vector<Type>& array, const std::string& name) {
  std::cout << "Loading " << name;
  array.resize(DATA_SIZE, 0);
  std::cout << ", Allocated";

  std::ifstream is(name.c_str(), std::ios::binary|std::ios::in);
  UnsignedType buf[1000];
  int loaded = 0;
  while (!is.eof()) {
    if (loaded >= array.size()) break;
    is.read((char *)&buf[0], sizeof(buf));
    int read_items = is.gcount() / sizeof(buf[0]);
    for (int i = 0; i < read_items; ++i) {
      endian_swap(buf[i], array[loaded]);
      loaded += 1;
    }
  }
  if (loaded != array.size()) {
    std::cerr << ", Bad file " << name
      << ", loaded " << loaded << " words but should be " << array.size()
      << std::endl;
    std::exit(1);
  }
}
...
int main(int argc, char argv[]) {
  ...
  for (int i = 0; i < ITERATIONS_NUM; ++i) {
    ...
    std::vector<int> offset(chunks), chunk_sizes(chunks, chunk_sz);
    ...
    for (std::vector<Type>::iterator r = merged.begin(); r != merged.end(); ++r) {
      ...
      std::vector<int>::iterator j = offset.begin();
      for (size_t i = 0; i < offset.size(); ++i, ++j)
        if (chunk_sizes[i] > 0 && unsorted[*j] < min) {
          min_i = i;
          min = unsorted[*j];
        }
      *r = min;
      offset[min_i] += 1;
      if (chunk_sizes[min_i] > 0)
        chunk_sizes[min_i] -= 1;
    }
