...
void __cdecl svc_listen_thread(LPVOID tParam)
{
  ...
  const char *iface=0, *config_iface=0, *config_run=0, *config_args=0;
      ...
      try
        {
          ...
          try
            {
              ...
              config_args=tag.get_attribute(CONFIG_ARGS);
            }
          catch(...)
            {
              tag.Unlock();
          ...
          logger::addfmt("'%s' on iface: '%s' on port: '%d' (handler: '%s', args: '%s')",
              config_iface,
              iface, port, config_run, config_args);
  ...
  while (0);
  logger::addfmt("Stopping '%s' on iface: '%s' on port: '%d' (handler: '%s', args: '%s')",
      config_iface,
      iface, port, config_run, config_args);
}
  ...
  try
    {
       ...
       try
         {
           con.init(rwstruct->tag.get_attribute(CONFIG_RUN), rwstruct->tag.get_attribute(CONFIG_ARGS));
         }
       catch(...)
         {
           rwstruct->tag.Unlock();
       ...
       while(con.is_alive())
       {
         ...
         if(-1 == (received = con.read(buf, sizeof(buf)-1))) 
           {
         ...
         logger::debugfmt("tid=%d. sendall, length=%ld", tid, received);
         if(!net::sendall(rwstruct->s, buf, received)) 
           {
         ...
         if((-1) == (received = net::recvall(rwstruct->s, buf, sizeof(buf)-1))) 
           {
         ...
         logger::debugfmt("tid=%d. con.write, length=%ld", tid, received);
         if(!con.write(buf, received)) 
           {
